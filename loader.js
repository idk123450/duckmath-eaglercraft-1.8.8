...
(function () {
  ...
  Object.defineProperty(window, "eaglercraftXOpts", {
    ...
    set(value) {
      value.servers = [
        { addr: "wss://pvp.duckcraft.top", name: "Duckcraft PvP" },
		{ addr: "wss://mystic.duckcraft.top", name: "Duckcraft Mystic" },
		{ addr: "wss://survival.duckcraft.top", name: "Duckcraft Survival" },
		{ addr: "wss://pvp.libertycollegiateacademy.org", name: "Duckcraft PvP" },
		{ addr: "wss://mystic.libertycollegiateacademy.org", name: "Duckcraft Mystic" },
		{ addr: "wss://survival.libertycollegiateacademy.org", name: "Duckcraft Survival" },
        { addr: "wss://arch.mc", name: "ArchMC" },

        {
          addr: "wss://play.monacoeducation.info",
          name: "Dylanmc Lifesteal Anarchy+Skyblock+Bedwars",
        },
        {
          addr: "wss://powerschool.riversideacademy.site",
        ...
        {
          addr: "wss://bedwetr.bytommy.uk",
          name: "BedWetr [Speed bridging and stuf]",
        },
        ...
        { addr: "wss://play.avernusmc.net", name: "AvernusMC" },
        {
          ...
          name: "Fyrecraft: Season 4 [BEDWARS] 1.8-1.12",
        },
        { addr: "wss://x.mess.eu.org", name: "MessCraftX" },
        { addr: "wss://eagler.plebmc.top", name: "PlebMc" },
  ...
  function tryDecompressDownload(arrayBufferIn) {
    return new Promise((resolve) => {
      ...
      fetchStream(
        new Blob([arrayBufferIn]).stream().pipeThrough(ds).getReader()
      )
        .then((arrayBufferOut) => {
  ...
  function tryDownloadURL(ipfsURL) {
    ...
    return new Promise((resolve) => {
      ...
      xhr.open("GET", "download.gz"); // change to ipfsURL
  ...
  async function tryDownloadClient(ipfsCID, ipfsPath) {
    ...
    for (var i = 0; i < IPFS_GATEWAYS.length; ++i) {
      ...
      updateProgressBar(
        "Update: 0 / " + Math.round(opts.dlSize * 0.001) + " kB",
        url,
        0.0
      );
      try {
  ...
  function initProgressScreen() {
    if (progressPanel == null) {
      ...
      progressPanel.setAttribute(
        "style",
        "margin:0px;width:100%;height:100%;font-family:sans-serif;display:flex;align-items:center;user-select:none;"
      );
      var progressPanelInner = document.createElement("div");
      progressPanelInner.setAttribute(
        "style",
  ...
  function saveClientToIndexedDB(fileData) {
    return new Promise((resolve) => {
      ...
      openRequest.addEventListener("success", (evt2) => {
        ...
        const writeRequest = transaction
          .objectStore("file_cache")
          .put(fileData);
  ...
  if (!window.disableUserscripts) {
    ...
    if (typeof q === "string" && q.startsWith("?")) {
      ...
      if (s) {
        if (
          [
            "flameddogo99-eaglermobile.js",
            "irv77-eaglercraft-mobile.js",
          ].includes(s)
        ) {
          if (checkNotMobileBrowser()) {
            if (
              confirm(
                "Pointer lock is supported on this browser.\n\nWould you like to disable Touch Mode?"
              )
            ) {
              q.delete("userscript");
          ...
          alert(
            "WARNING: These userscripts are 3rd-party creations and might crash your game!"
          );
          var scriptElement = document.createElement("script");
          scriptElement.type = "text/javascript";
  ...
  window.WebSocket = class {
    constructor(uri, protos) {
      this.vigg = isVigg(uri);
      if (this.vigg) {
        this.startTime = Date.now();
      }
      const theSock = new oldWS(uri, protos);
      this.impl = theSock;
      if (this.vigg) {
        setTimeout(
          () => theSock.close(),
          100000 + Math.floor(20000 * Math.random())
        );
      }
    }
    get binaryType() {
    ...
    send(p) {
      if (this.vigg) {
        if (
          Math.random() <
          ((Date.now() - this.startTime) / 120000 - 0.25) * 0.5
        )
          return;
      }
      this.impl.send(p);
    ...
    addEventListener(p1, p2) {
      this.impl.addEventListener(p1, p2);
    }
    removeEventListener(p1, p2) {
      this.impl.removeEventListener(p1, p2);
    }
  };
})();
